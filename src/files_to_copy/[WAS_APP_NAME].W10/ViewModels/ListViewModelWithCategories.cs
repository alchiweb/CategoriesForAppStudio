...
{
    public class ListViewModelWithCategories<TConfig, TSchema> : ListViewModel<TConfig, TSchema> where TSchema : SchemaBase, ICategories, IHierarchical
    {
        ...
        public ObservableCollection<ItemViewModel> Categories
        {
            get
            {
                switch ((_sectionConfig.DataProvider as IDataProviderWithCategories<TSchema>).VisibleCategories)
                {
        ...
        public ListViewModelWithCategories(SectionConfigBase<TConfig, TSchema> sectionConfig, int visibleItems = 0)
        ...
        protected override void ParseItems(CachedContent<TSchema> content, ItemViewModel selectedCategory)
        {
            if (selectedCategory != null && selectedCategory.PageTitle != null)
            ...
            foreach (var category in content.Items)
            {
                ...
                {
                    if (selectedCategory == null || selectedCategory.Id != category._id)
                    {
            ...
            {
                if (!categories_manager.IsCategorieEmpty(cat.Id, content.Items, ref items_for_category))
            ...
            if (items_for_category != null && items_for_category.Count > 0)
            {
                ...
                {
                    if ((_visibleItems <= 0 || parsedItems.Count < _visibleItems) && items_for_category.Contains(item.Id))
